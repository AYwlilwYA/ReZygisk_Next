...
if [ "$BOOTMODE" ] && [ "$KSU" ]; then
  ...
  if [ "$(which magisk)" ]; then
    ...
    ui_print "! Please uninstall Magisk before installing ReZygisk Next"
    abort    "*********************************************************"
  fi
...
ui_print "- Installing ReZygisk Next $VERSION"
...
if [ "$ARCH" = "x86" ] || [ "$ARCH" = "x64" ]; then
  if [ "$SUPPORTS_32BIT" = true ]; then
    ...
    extract "$ZIPFILE" 'lib/x86/libpayload.so' "$MODPATH/lib" true
    ...
    unzip -oj "$ZIPFILE" 'machikado.x86' -d "$MODPATH" >/dev/null 2>&1 || true
  fi
  ...
  if [ "$SUPPORTS_64BIT" = true ]; then
    ...
    unzip -oj "$ZIPFILE" 'machikado.x86_64' -d "$MODPATH" >/dev/null 2>&1 || true
  fi
else
  if [ "$SUPPORTS_32BIT" = true ]; then
    ...
    unzip -oj "$ZIPFILE" 'machikado.arm' -d "$MODPATH" >/dev/null 2>&1 || true
  fi
  ...
  if [ "$SUPPORTS_64BIT" = true ]; then
    ...
    unzip -oj "$ZIPFILE" 'machikado.arm64' -d "$MODPATH" >/dev/null 2>&1 || true
  fi
fi

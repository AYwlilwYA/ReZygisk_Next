...
/* INFO: 通信 socket 全部改用抽象命名空间（sun_path[0]='\0'），
         不再在 /proc/net/unix 暴露 /data/adb/rezygisk/ 文件系统路径。
         抽象名见 constants.h（伪装成系统/vendor 服务名）。 */
#define CONTROLLER_SOCKET ZN_MONITOR_SOCKET
...
static int create_daemon_socket(void) {
  /* INFO: 保持与旧实现一致的 sockcreate 上下文，使 socket inode 仍带
            zygote 标签，app 进程（untrusted_app/zygote 域）可正常 connect。
            抽象 socket 无文件系统路径，因此不再需要 chcon。 */
  set_socket_create_context("u:r:zygote:s0");

  return zn_stealth_create_hidden_socket(LP_SELECT(ZN_DAEMON_SOCKET_32, ZN_DAEMON_SOCKET_64));
}
...
void zygiskd_start(char *restrict argv[]) {
  ...
  if (impl.impl == None || impl.impl == Multiple) {
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &(uint8_t){ DAEMON_SET_ERROR_INFO }, sizeof(uint8_t));
    ...
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &msg_len, sizeof(msg_len));
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, msg, msg_len);

    exit(EXIT_FAILURE);
  } else {
    load_modules(&context);

    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &(uint8_t){ DAEMON_SET_INFO }, sizeof(uint8_t));
    ...
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &root_impl_len, sizeof(root_impl_len));
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, impl_name, root_impl_len);

    uint32_t modules_len = (uint32_t)context.len;
    unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &modules_len, sizeof(modules_len));

    for (size_t i = 0; i < context.len; i++) {
      uint32_t module_name_len = (uint32_t)strlen(context.modules[i].name);
      unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &module_name_len, sizeof(module_name_len));
      unix_datagram_sendto_abstract(CONTROLLER_SOCKET, context.modules[i].name, module_name_len);
    }
  ...
  while (1) {
    ...
    switch (action) {
      case ZygoteInjected: {
        unix_datagram_sendto_abstract(CONTROLLER_SOCKET, &(uint8_t){ ZYGOTE_INJECTED }, sizeof(uint8_t));

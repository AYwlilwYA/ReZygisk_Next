...
/* INFO: 向抽象命名空间 Unix datagram socket 发送数据。
         抽象 socket 不创建文件系统条目，/proc/net/unix 只显示为 @name，
         因此不会像旧实现那样暴露 /data/adb/rezygisk/init_monitor。 */
void unix_datagram_sendto_abstract(const char *restrict abstract_name, const void *restrict buf, size_t len) {
  ...
  /* 重试连接: monitor 可能还未创建 socket，等待最多 3 秒 */
  int socket_fd = -1;
  for (int retry = 0; retry < 30; retry++) {
    ...
    /* Socket 尚未创建，等待 100ms 后重试 */
    close(socket_fd);
    usleep(100000);
  }

  if (socket_fd == -1) {
    LOGE("connect to @%s: timeout after retries", abstract_name);
    set_socket_create_context("u:r:zygote:s0");
    return;
  }

  if (sendto(socket_fd, buf, len, 0, (struct sockaddr *)&addr, addr_len) == -1) {

...
/* INFO: Fallbacks so the memfd-based loading also builds on non-bionic hosts
           (e.g. the `make linux` dev target) where <sys/mman.h> may not pull
           in <linux/memfd.h>. */
#ifndef MFD_CLOEXEC
...
void *linker_load_library_manually(const char *lib_path, struct loaded_dep *out) {
  ...
  /* INFO: Copy the module .so into an anonymous memfd named "jit-cache" and
             load every segment from that memfd instead of the real file. This
             hides the real module path from /proc/self/maps, which would
             otherwise permanently show "/data/adb/modules/<mod>/zygisk/<abi>.so"
             (the core signature every Zygisk detection tool looks for).
             The memfd is a byte-for-byte copy, so all offsets and lengths used
             to load segments stay identical to the ones of the original file.
             Once both descriptors are closed, the mapping shows the disguised
             "/memfd:jit-cache (deleted)" path, indistinguishable from the ART
             JIT cache. */
  struct stat st;
  if (fstat(fd, &st) != 0) {
    PLOGE("fstat %s", lib_path);
  ...
  if (st.st_size <= 0) {
    LOGE("Invalid file size (%lld) for %s", (long long)st.st_size, lib_path);
  ...
  /* INFO: Try to stage the module .so into an anonymous memfd named "jit-cache"
             and load every segment from that memfd instead of the real file. If
             memfd_create is unavailable (old kernel, SELinux/seccomp denial, ...)
             or the copy fails, fall back to loading directly from the original
             file descriptor. The mapping then keeps the real module path, which
             is a degraded but functional mode. */
  memfd = (int)syscall(SYS_memfd_create, "jit-cache", MFD_CLOEXEC);
  if (memfd < 0) {
    PLOGE("memfd_create failed (%s), fallback to file mapping", strerror(errno));
  } else if (ftruncate(memfd, st.st_size) != 0) {
    PLOGE("ftruncate memfd failed (%s), fallback to file mapping", strerror(errno));

    close(memfd);
    memfd = -1;
  } else {
    char *copy_buf = malloc(64 * 1024);
    if (!copy_buf) {
      LOGE("Failed to allocate copy buffer for %s, fallback to file mapping", lib_path);
      ...
      while (done < st.st_size) {
        size_t want = (size_t)(st.st_size - done);
        if (want > 64 * 1024) want = 64 * 1024;

        ssize_t n = pread(fd, copy_buf, want, done);
        if (n <= 0) {
          LOGE("Failed to read %s at offset %lld: %s", lib_path, (long long)done, strerror(errno));
        ...
        size_t copied = 0;
        while (copied < (size_t)n) {
          ssize_t w = write(memfd, copy_buf + copied, (size_t)n - copied);
          if (w <= 0) {
            LOGE("Failed to write memfd for %s (%s), fallback to file mapping", lib_path, strerror(errno));
  ...
  if (memfd >= 0) {
    ...
    /* INFO: Seal the memfd so its backing data can never be shrunk, grown or
               rewritten afterwards. The segment mappings are MAP_PRIVATE, so
               they remain writable through copy-on-write. */
  ...
  ElfW(Ehdr) eh;
  if (pread(load_fd, &eh, sizeof eh, 0) != sizeof eh) {
  ...
  if (pread(load_fd, phdr, phdr_sz, eh.e_phoff) != (ssize_t)phdr_sz) {
  ...
  for (int i = 0; i < eh.e_phnum; i++) {
    ...
    if (_linker_load_one_segment(load_fd, &phdr[i], bias, 0) != 0) {
  ...
  /* INFO: Close the memfd (if used) and the original file. In the memfd path
             the segment mappings keep a reference to the memfd inode, so
             /proc/self/maps shows the disguised "/memfd:jit-cache (deleted)"
             path instead of the real module path. In the fallback path the
             mappings reference the original file and keep its real path. */
  if (memfd >= 0) close(memfd);
  close(fd);

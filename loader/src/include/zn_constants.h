...
/* Socket 抽象名（伪装，/proc/net/unix 只见 @name，不含 /data/adb/rezygisk 路径）
   实际加解密存储在 zn_strings.h / generate_strings.py（ZN_SOCKET_DAEMON_32/64、ZN_SOCKET_MONITOR）。
   此处为编译期兜底常量，必须与 zygiskd/src/constants.h 一致。 */
#ifdef __LP64__
    #define ZN_SOCKET_NAME  "resmon_agent"
#else
    #define ZN_SOCKET_NAME  "engine_state_ctl"
#endif

#define ZN_SOCKET_CONTROLLER "criticallog_evt"

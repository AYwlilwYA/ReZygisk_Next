...
bool trace_zygote(int pid, bool tango_flag) {
  ...
  if (version.major > 3 || (version.major == 3 && version.minor >= 8)) {
    ...
    if (tango_flag) {
      ...
      const char *lib_path = zn_strings_get(ZN_PATH_LIBZYGISK_32);
  ...
  if (STOPPED_WITH(SIGSTOP, PTRACE_EVENT_STOP)) {
    const char *lib_path = LP_SELECT(zn_strings_get(ZN_PATH_LIBZYGISK_32), zn_strings_get(ZN_PATH_LIBZYGISK_64));

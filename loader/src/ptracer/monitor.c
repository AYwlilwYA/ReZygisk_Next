...
/* INFO: Fallbacks so the memfd availability probe also builds on non-bionic
           hosts (e.g. the `make linux` dev target) where <sys/mman.h> may not
           pull in <linux/memfd.h>. */
#ifndef MFD_CLOEXEC
...
/* INFO: memfd availability. When memfd_create fails at runtime, the module .so
           is loaded directly from the file (see csoloader linker fallback), so
           /proc/self/maps keeps the real module path. We expose that degraded
           mode both in state.json and in module.prop's description. */
static bool g_memfd_available = true;
#define MEMFD_FALLBACK_MARKER " [⚠️ memfd 回退模式，模块路径隐藏降级]"

static bool check_memfd_available(void) {
  int fd = (int)syscall(SYS_memfd_create, "mtest", 0);
  if (fd < 0) {
    LOGW("memfd_create unavailable (%s), module will use file fallback", strerror(errno));
...
/* INFO: Append MEMFD_FALLBACK_MARKER right before the first newline so it lands
           at the end of the module.prop description value. Idempotent. */
static void ensure_memfd_marker(char *s) {
  if (strstr(s, MEMFD_FALLBACK_MARKER)) return;

  char *nl = strchr(s, '\n');
  if (!nl) return;

  size_t mlen = strlen(MEMFD_FALLBACK_MARKER);
  size_t pos = (size_t)(nl - s);

  memmove(s + pos + mlen, s + pos, strlen(s) - pos + 1);
  memcpy(s + pos, MEMFD_FALLBACK_MARKER, mlen);
}

/* INFO: Remove any previously appended MEMFD_FALLBACK_MARKER. Idempotent. */
static void strip_memfd_marker(char *s) {
  char *found = strstr(s, MEMFD_FALLBACK_MARKER);
  if (!found) return;

  size_t mlen = strlen(MEMFD_FALLBACK_MARKER);

  memmove(found, found + mlen, strlen(found + mlen) + 1);
}
...
bool rezygiskd_listener_init() {
  ...
  /* INFO: 绑定抽象命名空间 socket（sun_path[0]='\0'），不再创建
            /data/adb/rezygisk/init_monitor 文件，app 视角 /proc/net/unix
            只见 @criticallog_evt。 */
  ...
  const char *sock_name = zn_strings_get(ZN_SOCKET_MONITOR);
  addr.sun_path[0] = '\0';
...
static bool ensure_daemon_created(bool is_64bit) {
  ...
  if (pid == 0) {
    ...
    strcpy(daemon_name, "/data/adb/modules/rezygisk/bin/zygiskd");
...
#define PRE_INJECT(abi, is_64)                                        \
  if (strcmp(program, APP_PROCESS_ ## abi) == 0) {                    \
    tracer = "/data/adb/modules/rezygisk/bin/zygisk-ptrace" # abi;                             \
    is_tango = false;                                                 \
                                                                      \
...
#define PRE_INJECT_TANGO                                           \
  if (strcmp(program, "/system_ext/bin/tango_translator") == 0) {  \
    tracer = "/data/adb/modules/rezygisk/bin/zygisk-ptrace32";                              \
    is_tango = true;                                               \
                                                                   \
...
static bool update_status(const char *message) {
  ...
  if (environment_information64.root_impl || environment_information32.root_impl) {
    ...
    fprintf(json, "  \"memfd\": \"%s\",\n", g_memfd_available ? "ok" : "fallback");
    ...
    if (status64.daemon_running || status32.daemon_running)
      fprintf(json, "  },\n");
    else
      fprintf(json, "  }\n");
    ...
    if (status64.daemon_running || status32.daemon_running) {
      ...
      if (status64.daemon_running) {
        ...
        fprintf(json, "]\n");
        fprintf(json, "    }");
        if (status32.daemon_running) fprintf(json, ",\n");
        else fprintf(json, "\n");
      }

      if (status32.daemon_running) {
        fprintf(json, "    \"32\": {\n");
      ...
      if (status64.daemon_running) {
        ...
        if (status32.daemon_running && status32.zygote_injected) fprintf(json, ",\n");
        else fprintf(json, "\n");
      }
      if (status32.daemon_running && status32.zygote_injected) {
...
static bool prepare_environment() {
  ...
  while (fgets(line, sizeof(line), orig_prop) != NULL) {
    if (strncmp(line, "description=", strlen("description=")) == 0) {
      ...
      /* INFO: Reflect the memfd fallback state in the description. When memfd is
                 available, drop any leftover marker (e.g. from a previous fallback
                 boot); when it is not, make sure the marker is present. */
      if (g_memfd_available) strip_memfd_marker(post_section);
      else ensure_memfd_marker(post_section);
...
void init_monitor(int argc, char **argv) {
  ...
  g_memfd_available = check_memfd_available();
  LOGI("memfd mode: %s", g_memfd_available ? "ok" : "fallback");

  /* 隐藏性: 进程名伪装 */
  const char *fake_names[] = { "netd", "logd", "installd", "servicemanager" };
  size_t idx = (size_t)getpid() % (sizeof(fake_names) / sizeof(fake_names[0]));
  prctl(PR_SET_NAME, fake_names[idx], 0, 0, 0);
  prctl(PR_SET_DUMPABLE, 0, 0, 0, 0);

  /* 隐藏性: 用伪装名覆盖 /proc/self/cmdline */
  if (argc > 0 && argv[0]) {
    char comm[16];
...
int send_control_command(enum rezygiskd_command cmd) {
  ...
  /* INFO: 与 rezygiskd_listener_init 相同，发往抽象命名空间 socket。 */
  struct sockaddr_un addr = {
    .sun_family = AF_UNIX,
    .sun_path = { 0 }
  };

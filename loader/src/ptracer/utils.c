...
/* INFO: Only used for Tango, only for AArch64 */
  ...
  /* INFO: 用 process_vm_writev/readv 代替 ptrace PEEKDATA/POKEDATA
           绕过 /proc/pid/status 中 TracerPid 相关的内存访问检测 */
  bool ptrace_poke_u32(pid_t pid, uintptr_t addr, uint32_t value) {
    ...
    if (read_proc(pid, aligned, &data, sizeof(data)) != (ssize_t)sizeof(data)) {
      PLOGE("read_proc at 0x%" PRIxPTR, addr);
      return false;
    }
    ...
    if (write_proc(pid, aligned, &patched, sizeof(patched)) != (ssize_t)sizeof(patched)) {
      PLOGE("write_proc at 0x%" PRIxPTR, addr);
      return false;
    }

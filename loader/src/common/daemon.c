...
int rezygiskd_connect(uint8_t retry) {
  /* INFO: 连接抽象命名空间 socket（sun_path[0]='\0'），名称与 zygiskd
            端一致（见 zygiskd/src/constants.h），通过 zn_strings 加密存储。
            不再暴露 /data/adb/rezygisk/cp32.sock 之类的文件系统路径。 */
  const char *sock_name = LP_SELECT(zn_strings_get(ZN_SOCKET_DAEMON_32),
                                    zn_strings_get(ZN_SOCKET_DAEMON_64));

...
STRINGS = [
    ...
    # ── 抽象 socket 名（伪装，/proc/net/unix 只见 @name）──
    # 必须与 zygiskd/src/constants.h 中 ZN_DAEMON_SOCKET_32/64、ZN_MONITOR_SOCKET 一致
    "engine_state_ctl",     # ZN_SOCKET_DAEMON_32 — 原 cp32.sock
    "resmon_agent",         # ZN_SOCKET_DAEMON_64 — 原 cp64.sock
    "criticallog_evt",      # ZN_SOCKET_MONITOR   — 原 init_monitor
]

assert len(STRINGS) == 27, f"Expected 27 strings, got {len(STRINGS)}"

...
PAYLOAD_DONE = $(OBJ_DIR)/payload/.done
...
PAYLOAD_INPUTS = common.mk payload/Makefile \
        $(shell find payload/src payload/include -type f 2>/dev/null | sort)
...
# 混淆构建 (需要 OLLVM/Hikari 工具链)
obfuscate:
	$(MAKE) OBFUSCATE=1 BUILD_TYPE=release BUILD_DIR=$(BUILD_DIR) build
...
$(PAYLOAD_DONE): $(PAYLOAD_INPUTS)
	$(MAKE) -C payload BUILD_TYPE=$(BUILD_TYPE) BUILD_DIR=$(BUILD_DIR)
	@mkdir -p $(dir $@)
	@touch $@

$(MODULE_DONE): $(LOADER_DONE) $(ZYGISKD_DONE) $(PAYLOAD_DONE) $(MODULE_INPUTS)
	...
	@for arch in $(ARCHS); do                                                                                  \
		...
		if [ -f $(OBJ_DIR)/payload/$$arch/stripped/libpayload.so ]; then                                       \
			cp $(OBJ_DIR)/payload/$$arch/stripped/libpayload.so $(MODULE_OUT)/lib/$$arch/libpayload.so;         \
		fi                                                                                                    \
	done
...
$(ZIP_FILE): $(MODULE_DONE)
	...
	@python3 scripts/pack_zip.py "$(MODULE_OUT)" "$(ZIP_FILE)"
...
clean:
	...
	$(MAKE) -C payload clean BUILD_DIR=$(BUILD_DIR) 2>/dev/null || true
